...
- 🚧 Tauri desktop (macOS / Windows / Linux)
  ...
  - 🚧 Tier-A polish — ✅ system tray (menu-bar icon) + native menu bar, hide-to-tray on window close (#8, all Rust-side; webview IPC surface stays empty); ⬜ Touch ID/Hello unlock; ⬜ auto-update (Tauri built-in updater chosen; plugin + signed-manifest endpoint pending a release channel)
  - ✅ Desktop bug fixes (#26) — Send share-URL "Copy URL" now builds from the configured server base instead of `tauri://localhost`; `window.confirm/alert/prompt` (which wry doesn't render — they silently no-op in the webview) replaced app-wide with a promise-based in-app `<DialogHost/>` modal, restoring every destructive flow (Send delete, move-to-trash, member removal, etc.); macOS-conformant app icon (transparent padding per Apple's icon grid, separate `icon-src.svg` master)
  - 🚧 Code signing + notarization — plumbing landed (`make desktop-release` + `desktop-sign-check` preflight; env-driven Developer ID signing + App Store Connect API-key notarization, secrets out of repo; setup guide in `clients/desktop/README.md`). Needs the operator's Developer ID cert + API key to produce an actual signed/notarized build (#8).
