...
fn read_u64_le(p: &mut &[u8]) -> Result<u64> {
    ...
    let v = u64::from_le_bytes(p[..8].try_into().map_err(|_| Error::InvalidEncoding("manifest u64".into()))?);
    *p = &p[8..];
    Ok(v)
}
...
fn read_u32_le(p: &mut &[u8]) -> Result<u32> {
    ...
    let v = u32::from_le_bytes(p[..4].try_into().map_err(|_| Error::InvalidEncoding("manifest u32".into()))?);
    *p = &p[4..];
    Ok(v)
}
